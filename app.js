(function () {
  ...
  function openReservation(venueId, time) {
    ...
    if (!venue || venue.unavailable.includes(time)) return;
  ...
  function userView() {
    ...
    const cards = filtered.map((venue) => `
      <article class="card venue-card">
        ...
        <div class="venue-body">
          ...
          <div class="slot-row">
            ${venue.slots.map((slot) => `<button class="slot ${venue.unavailable.includes(slot) ? "unavailable" : ""}" data-reserve="${venue.id}" data-time="${slot}" ${venue.unavailable.includes(slot) ? "disabled" : ""}>${slot}</button>`).join("")}
          </div>
          <div class="price-row">
            <div class="price"><small>A partir de</small>${money(venue.price)}</div>
            <button class="button secondary small" data-details="${venue.id}">Ver espaço</button>
          </div>
        </div>
      </article>`).join("");
    ...
    return `
      <section class="hero">
        <article class="hero-card">
          ...
          <div class="hero-actions">
            <button class="button primary" data-scroll="venues">Explorar horários</button>
            <button class="button ghost" data-show-reservations>Minhas reservas</button>
          </div>
        </article>
      ...
      <div class="section-title"><div><h2>Próxima reserva</h2><p>Acompanhamento antes da integração financeira.</p></div></div>
      <section class="card">
    ...
    return `
      <section class="hero">
        <article class="hero-card">
          ...
          <div class="hero-actions">
            <button class="button primary" data-action="new-block">Bloquear horário</button>
            <button class="button ghost" data-action="new-space">Cadastrar espaço</button>
          </div>
        </article>
        <article class="hero-card kpi-card">
      ...
      <div class="section-title"><div><h2>Agenda de hoje</h2><p>Arena Central · Quadra Society 1</p></div><button class="button secondary small" data-action="calendar">Abrir calendário</button></div>
      <section class="card">
        <div class="calendar-strip">
          ${[["SEG",3],["TER",4],["QUA",5],["QUI",6],["SEX",7],["SÁB",8],["DOM",9]].map(([d,n], i) => `<div class="day ${i === 3 ? "active" : ""}"><small>${d}</small><strong>${n}</strong></div>`).join("")}
        </div>
        <div class="timeline">
    ...
    return `
      <section class="hero">
        <article class="hero-card">
          ...
          <div class="hero-actions">
            <button class="button primary" data-action="review-next">Analisar próximo parceiro</button>
            <button class="button ghost" data-action="export">Exportar relatório</button>
          </div>
        </article>
        <article class="hero-card kpi-card">
          <div><span class="eyebrow">Implantação</span><div class="kpi-value">0.1</div><div class="kpi-label">telas e fluxos iniciais</div></div>
      ...
      <section class="stats-grid">
        ...
        <div class="stat"><strong>1</strong><span>aguardando análise</span></div>
          ...
          ${state.partners.map((partner) => `
            <div class="list-item">
              ...
              <div class="item-actions">
                ${partner.status !== "Aprovado" ? `<button class="button secondary small" data-approve="${partner.id}">Aprovar</button>` : ""}
                <button class="button ghost small" data-review="${partner.id}">Detalhes</button>
              </div>
            </div>`).join("")}
        </div>
      </section>
  ...
  function bindDynamicEvents() {
    ...
    if (search) search.addEventListener("input", (event) => { state.search = event.target.value; render(); document.getElementById("venueSearch")?.focus(); });
    ...
    document.querySelectorAll("[data-details]").forEach((button) => button.addEventListener("click", () => showToast("Detalhes completos do espaço entram no próximo ciclo.")));
    ...
    document.querySelectorAll("[data-show-reservations]").forEach((button) => button.addEventListener("click", () => showToast("A seção de reservas está logo abaixo dos espaços.")));
    document.querySelectorAll("[data-action]").forEach((button) => button.addEventListener("click", () => showToast(`Ação demonstrativa: ${button.textContent.trim()}.`)));
    document.querySelectorAll("[data-review]").forEach((button) => button.addEventListener("click", () => showToast("Tela detalhada do parceiro será criada no próximo ciclo.")));
